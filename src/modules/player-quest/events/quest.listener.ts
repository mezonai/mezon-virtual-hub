...
import { QuestCompletedPayload, QuestProgressPayload } from "./quest.events";

@Injectable()
export class QuestListener implements OnModuleInit {
  constructor(private readonly questProgressService: PlayerQuestProgressService) {}

  onModuleInit() {
    GlobalEventCommon.on(EventTypes.QUEST, (data: QuestProgressPayload) => {
      this.questProgressService.addProgress(
        data.userId,
