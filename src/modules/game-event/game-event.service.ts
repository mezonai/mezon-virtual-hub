...
import { CreateEventGameDto, GameEventResDto, UpdateEventGameReqDto } from './dto/game-event.dto';
...
export class GameEventService extends BaseService<GameEventEntity> {
    ...
    async findAll() {
        const events = await this.gameEventRepository.find({ order: { start_time: 'ASC' } });
        return plainToInstance(GameEventResDto, events);
    }

    async createEvent(dto: CreateEventGameDto) {
        const targetUser = await this.userRepository.findOne({ where: { id: dto.target_user_id } });

        if (!targetUser) {
            throw new NotFoundException(`Target User with ID ${dto.target_user_id} not found`);
        }
        ...
        return await this.gameEventRepository.save(event);
    }

    async updateEvent(id: string, dto: UpdateEventGameReqDto): Promise<GameEventEntity> {
        const event = await this.gameEventRepository.findOneByOrFail({ id });

        if (dto.target_user_id) {
            event.target_user = await this.userRepository.findOneByOrFail({ id: dto.target_user_id });
        }
